...
// ---------------------------------------------------------------------------
// Demo / Seed data (used until real on-chain indexer is ready)
// ---------------------------------------------------------------------------

export function generateDemoLeaderboard(): LeaderboardEntry[] {
  const names = [
    "SourdoughKing", "CryptoFerment", "BreadWinner", "DoughRiser",
    "CrustPunk", "BakerDAO", "YeastMaster", "GoldenLoaf",
    "StarterChef", "FlourPower", "OvenKeeper", "WheatWarden",
    "GrainGuard", "RyeRider", "SeedSower", "MillHand",
    "KneadKnight", "ProofPilot", "BakeOrDie", "CrumbLord",
  ];

  return names.map((name, i) => {
    const balance = Math.round((20 - i) * 500_000 * (0.5 + Math.random()));
    const days = Math.round(Math.max(1, 400 - i * 20 + Math.random() * 30));
    const hs = Math.round(Math.max(0, 60 - i * 3 + Math.random() * 5));
    const input: CrustScoreInput = {
      balance,
      daysFermenting: days,
      handshakesCompleted: hs,
      disputesLost: i > 15 ? 1 : 0,
      handshakesCancelled: i > 12 ? 2 : 0,
      handshakesTotal: hs + (i > 12 ? 2 : 0),
    };
    const score = calculateCrustScore(input);
    // Generate deterministic-looking wallet address
    const addr = `${(i + 7).toString(16).padStart(4, "0")}${name.slice(0, 4)}${"x".repeat(32)}`.slice(0, 44);

    return {
      address: addr,
      displayName: name,
      avatar: "/sour-logo.png",
  ...
  }).sort((a, b) => b.crustScore - a.crustScore);
}
